...
local PLATFORM_TYPES = {
    ...
    ["Mac"] = {
        ["defines"] = {
            ...
            "PLATFORM_APPLE",
            "PLATFORM_UNIX",
            -- There is no Unreal/Core/Mac/ platform-header dir (only Linux/Unix/
            -- Windows exist), so reuse the Linux uber-platform headers. They are
            -- POSIX/Unix-based and compile under clang; macOS-specific behaviour
            -- is layered on top via PLATFORM_MAC/PLATFORM_APPLE where needed (P3+).
            "OVERRIDE_PLATFORM_HEADER_NAME=Linux",
            "UBT_COMPILED_PLATFORM=Mac",
            "printf_s=printf",
            "DLLEXT=.dylib",
            -- TCHAR is char16_t on Unix → STR/TEXT/WIDETEXT must emit u"" (char16_t), not L"" (wchar_t).
            "PLATFORM_TCHAR_IS_CHAR16=1"
        },
        ["cxflags"] = {
            "clang::-fno-delete-null-pointer-checks"
        }
