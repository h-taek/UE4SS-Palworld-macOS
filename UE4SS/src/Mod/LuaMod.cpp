...
#1: UnregisterHook(string UFunction_Name, integer PreCallbackId, integer PostCallbackId))"};
            ...
            lua.register_function("DumpStaticMeshes", []([[maybe_unused]] const LuaMadeSimple::Lua& lua) -> int {
                // GUI::Dumpers lives in src/GUI and is only compiled when a UI backend
                // is enabled (HAS_UI). With --ue4ssUI=None it is unavailable.
                // TODO(P4): make the dumpers UI-independent so they work headless on macOS.
#ifdef HAS_UI
