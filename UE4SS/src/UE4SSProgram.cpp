...
#ifdef __APPLE__
#include <atomic>
#include <DynamicOutput/StderrDevice.hpp>
...
#ifdef __APPLE__
            // macOS: iCloud sandboxing often blocks the file device from writing UE4SS.log.
            // Add a stderr sink so the launcher script (smoke-load.log 2>&1) always captures
            // the full UE4SS log regardless of filesystem access.
            Output::set_default_devices<Output::StderrDevice>();
            fprintf(stderr, "[UE4SS-mac] StderrDevice registered (log mirror active)\n");
            fflush(stderr);
#endif
...
#define UE4SS_COMPILER SYSSTR("MSVC")
        ...
        {
#ifdef __APPLE__
            // Diagnostic: the constructor's error path is otherwise invisible on macOS
            // (Output devices may not be registered yet). e.what() is const char*.
            fprintf(stderr, "[UE4SS-mac] ctor threw: %s\n", e.what());
            fflush(stderr);
#endif
    ...
    auto UE4SSProgram::init() -> void
    {
        ...
        try
        {
            ...
            // [macOS bring-up] B-gate hard evidence: prove the installed ProcessEvent
            // hook actually *fires* each frame (not just that it installed cleanly) by
            // registering a throttled diagnostic pre-callback. The hooked trampoline
            // (Unreal::Hooks HookedProcessEvent) iterates ProcessEventPreCallbacks on
            // every call, so a single callback that logs is enough to nail it down.
            // Gated behind UE4SS_MAC_DIAG (default OFF) — quiet by default, re-enable
            // for hook debugging (e.g. AutoHatch bring-up). Same gate as the reloc [DIAG] logs.
            if (const char* diag = std::getenv("UE4SS_MAC_DIAG"); diag && diag[0] && diag[0] != '0')
            {
                static std::atomic<uint64_t> s_pe_calls{0};
                Unreal::Hook::RegisterProcessEventPreCallback(
                        []([[maybe_unused]] Unreal::UObject* Context, Unreal::UFunction* Function, [[maybe_unused]] void* Parms) {
                            const auto n = s_pe_calls.fetch_add(1) + 1;
                            // First 20 calls: log each UFunction name. After that: 1 line / 1000 calls.
                            if (n <= 20 || (n % 1000) == 0)
                            {
                                const auto fn = Function ? to_system(Function->GetName()) : SYSSTR("<null>");
                                Output::send(SYSSTR("[UE4SS-mac][PE] #{} {}\n"), n, fn);
                            }
                        });
                Output::send(SYSSTR("[UE4SS-mac][PE] diagnostic pre-callback registered (B-gate hard evidence)\n"));
            }
#endif
...
#ifndef __APPLE__
        // macOS 샌드박스는 iCloud/dylib 경로의 directory_iterator를 "Operation not
        // permitted"로 거부한다. 이 게임별 설정 스캔은 macOS에선 건너뛰고, 아래에서
        // UE4SS를 샌드박스 컨테이너에 루팅한다.
...
#ifdef __APPLE__
        // 샌드박스: dylib는 iCloud(쓰기불가) 경로에 있다. 작업 데이터는
        // 샌드박스 컨테이너($HOME = .../Containers/com.pocketpair.palworld.mac/Data)에 둔다.
        if (const char* home = getenv("HOME"))
        {
            std::filesystem::path container_ue4ss = std::filesystem::path(home) / "UE4SS";
            m_working_directory = container_ue4ss;
            m_mods_directory = container_ue4ss / "Mods";
            m_settings_path_and_file = container_ue4ss;
            m_log_directory = container_ue4ss;
            m_object_dumper_output_directory = container_ue4ss;
            std::error_code ec;
            std::filesystem::create_directories(container_ue4ss / "Mods", ec);
        }
#endif
