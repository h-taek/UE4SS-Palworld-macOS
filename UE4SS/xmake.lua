...
target(projectName)
    ...
    elseif is_plat("macosx") then
        add_files("src/Platform/Darwin/EntryDarwin.cpp")
        add_shflags("-Wl,-install_name,@rpath/libUE4SS.dylib")
        -- P3: link the arm64 inline hooker.
        add_deps("MacHook")
    end

    -- capstone 정적 링크: on_load 내에서 is_plat 확인 → brew .a를 shflags로 추가
    -- libcapstone.dylib 동적 의존 없이 self-contained dylib 생성.
    on_load(function(target)
        if is_plat("macosx") then
            local capstone_prefix = os.iorun("brew --prefix capstone"):trim()
            target:add("shflags", capstone_prefix .. "/lib/libcapstone.a", {force = true})
        end
    end)
    ...
    -- polyhook_2 is x86-only / no macOS build; all PLH:: usage in UE4SSProgram.cpp
    -- is already #ifdef WIN32-guarded, so macOS needs only glaze.
    if is_plat("macosx") then
