        ...
        auto static setup_member_functions(LuaMadeSimple::Lua::Table& table) -> void
        {
            ...
            if constexpr (is_final == LuaMadeSimple::Type::IsFinal::Yes)
            {
                table.get_metamethods().create(LuaMadeSimple::Lua::MetaMethod::Index, [](const LuaMadeSimple::Lua& lua) -> int {
                    ...
                    // TODO(P4): RemoteObject has no 'get_pusher_callable' member in upstream
                    // UE4SS — this Index metamethod path is only valid when never instantiated.
                    // clang on macOS instantiates it, so stub it out to compile. Restore real
                    // behaviour when the Lua RemoteObject pusher design is ported.
                    (void)lua;
                    return 0;
#else
