...
namespace RC::Helper::Casting
{
    ...
    // macOS: UE4SS runs in-process (injected dylib), so there is no separate
    // remote process to read from via process_vm_readv (a Linux-only syscall).
    // We can dereference directly since the target memory is in our own address space.
    // TODO(P4): add a real pointer-validity guard (mach_vm_read / sigsetjmp probe).
    template <typename To, typename From>
