...
// macOS has no <elf.h>; the only ELF symbol used below is PF_R (program-header
// read flag). Define it to its ELF value so the shared scalar scan logic
// compiles. The phdrs set is empty on macOS today, so this path is inert.
// TODO(P4): real Mach-O segment/permission walk instead of ELF phdrs.
#ifndef PF_R
#define PF_R 0x4
#endif
#else
